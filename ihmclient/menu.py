...
def connexion():
    gamevs = GameVs()
    gamevs.game_init(surface, TILE_SIZE, username_input.get_value(), ip_input.get_value(), port_input.get_value(), joueur_input.get_value()[0])
...
def menu_loop():
    ...
    play_vsplayer = pygame_menu.Menu(
        theme=menu_theme,
        height=int(WINDOW_SIZE[1] * WINDOW_SCALE),
        width=int(WINDOW_SIZE[0] * WINDOW_SCALE),
        title='Vs_Player'
    ...
    joueur_input = play_vsplayer.add.selector('Nombres Joueurs : ', ["2", "3", "4"])
    ...
    play_menu.add.button('Start',
                         run_game)
    play_menu.add.button('Vs Player', play_vsplayer)
